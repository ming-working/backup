...
class AnimatedCircularChartState extends State<AnimatedCircularChart>
  ...
  final List<TextPainter> _labelPainter =
      List.generate(6, (i) => TextPainter(), growable: false);
  ...
  void _updateLabelPainter() {
    for (int i = 0; i < widget.labelStyle!.length; i++) {
      if (widget.holeLabel != null) {
        TextStyle? labelStyle = widget.labelStyle![i];
        _labelPainter[i]
          ..text = TextSpan(style: labelStyle, text: widget.holeLabel![i])
          ..textDirection = Directionality.of(context)
          ..textScaleFactor = MediaQuery.of(context).textScaleFactor
          ..textAlign = TextAlign.center
          ..layout();
      } else {
        _labelPainter[i].text = null;
      }
    }
  }
